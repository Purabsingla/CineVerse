...
const TrendingShows = forwardRef((props, ref) => {
  const navigate = useNavigate();
  // Total number of items
  const API_KEY = String(process.env.REACT_APP_API_KEY).trim();
  const timeWindow = "week"; // or "day"
  const url = `https://api.themoviedb.org/3/trending/tv/${timeWindow}?api_key=${API_KEY}`;
  ...
  return (
    <div
      className="pt-8  bg-deep-space-2 bg-opacity-80 transition-all"
      ref={ref}
    >
      {/* Heading Section */}
      <div className="py-8">
